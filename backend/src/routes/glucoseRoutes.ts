...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    // Time In Range : 70-180 mg/dL (cible internationale)
    ...
    // HbA1c estimé : formule internationale (ADAG)
    // HbA1c = (average + 46.7) / 28.7
    const estimatedHbA1c = Math.round(((average + 46.7) / 28.7) * 10) / 10

    // Score d'observance : nb de jours avec au moins 1 mesure / total jours
    const uniqueDays = new Set(
      readings.map((r: any) => new Date(r.measured_at).toDateString())
    ).size
    const adherenceScore = Math.round((uniqueDays / days) * 100)

    // Tendance : compare moyenne des 7 derniers jours vs 7 jours précédents
    const now = new Date()
...
router.get('/recommendations', async (req: Request, res: Response) => {
  try {
    ...
    // Dernières 24h pour alertes critiques
    ...
    // 30 derniers jours pour tendances
    ...
    // Alerte si pas de mesure depuis 24h
    if (recent.length === 0) {
      alerts.push({
        type: 'reminder',
        icon: '🔔',
    ...
    if (allReadings.length >= 5) {
      ...
      if (tir >= 70) {
        ...
        recommendations.push({
          type: 'info',
          icon: '💪',
          title: 'Bon contrôle, améliorable',
      ...
      if (afterMeal.length >= 3) {
        ...
        if (avgAfter > 160) {
          recommendations.push({
            type: 'info',
            icon: '🚶',
            title: 'Marche post-repas recommandée',
      ...
      recommendations.push({
        type: 'info',
        icon: '📊',
        title: 'Continuez à enregistrer vos mesures',
